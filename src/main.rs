...
fn read_temp() -> String {
    // Pobieramy listę komponentów (czujników temperatury) i od razu ją odświeżamy
    let components = Components::new_with_refreshed_list();

    // Zazwyczaj procesor (CPU) jest na początku listy
...
async fn main() {
    // --- ŁADOWANIE ZASOBÓW ---
    let logo_texture = load_texture("assets/logo.png")
        .await
        .expect("Failed to load logo.png");
    let bg_texture = load_texture("assets/background.png")
        .await
        .expect("Failed to load background.png");
    let my_font = load_ttf_font("assets/my_font.otf")
        .await
    ...
    let update_interval = 5; // aktualizuj co 5 iteracji (co 500ms zamiast co 100ms)
    loop {
        // --- LOGIKA SYSTEMOWA ---
        let mut cpu_val = 0.0;
        ...
        if let Ok(mem) = read_to_string("/proc/meminfo") {
            ...
            for l in mem.lines() {
                if l.starts_with("MemTotal:") {
                    t = l.split_whitespace().nth(1).unwrap().parse().unwrap();
                }
                if l.starts_with("MemAvailable:") {
                    a = l.split_whitespace().nth(1).unwrap().parse().unwrap();
                }
            }
        ...
        // --- sieć sysinfo ---
        let mut current_total: u128 = 0;

        networks.refresh(false); // odświeżamy dane (bez true = nie odświeżamy listy interfejsów)

        for (interface_name, interface) in &networks {
            if interface_name == "lo" {
                continue;
            }
            // kumulatywne bajty od startu systemu
            current_total = current_total
                .saturating_add(interface.total_received() as u128)
                .saturating_add(interface.total_transmitted() as u128);
        }

        let mut net_val = 0.0_f32;

        if last_net_bytes > 0 && current_total >= last_net_bytes {
            // delta bajtów w 100 ms → ×10 = bajty/s → /1024 = KB/s
            let delta = current_total - last_net_bytes;
            net_val = (delta as f32 * 10.0) / 1024.0;
        }

        last_net_bytes = current_total;
        ...
        draw_ftext(
            &format!("TOTAL RAM: {:.2} GB", total_ram_gb),
            20.0,
            125.0,
            18.0,
            WHITE,
        ...
        draw_chart(
            &format!("CPU LOAD: {:.1}% | TEMP: {}", cpu_val, read_temp()),
            170.0,
            &cpu_history,
            ORANGE,
            100.0,
        ...
        draw_chart(
            &format!("RAM USAGE: {:.1}% ({:.2} GB used)", ram_pct, ram_used_gb),
            320.0,
            &ram_history,
            SKYBLUE,
            100.0,
        ...
        let net_display = if net_val >= 1024.0 {
            format!("{:.1} MB/s", net_val / 1024.0)
        } else {
            format!("{:.1} KB/s", net_val)
        };

        draw_chart(
            &format!("NET SPEED: {}", net_display),
            470.0,
            &net_history,
            PURPLE,
            1000.0, 
        ...
        draw_line(20.0, 655.0, 500.0, 655.0, 1.0, GRAY);
        ...
        let uptime = read_to_string("/proc/uptime")
            ...
            .parse::<f32>()
            .unwrap_or(0.0);
        let procs_info = read_to_string("/proc/loadavg")
            .unwrap_or_default()
            .split_whitespace()
            .nth(3)
            .unwrap_or("N/A")
        ...
        draw_ftext(
            &format!("SYSTEM THREADS: {}", procs_info),
            250.0,
            930.0,
            20.0,
            LIGHTGRAY,
